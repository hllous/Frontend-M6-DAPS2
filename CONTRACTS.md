...
| Indicators | Endpoint/filter/semantic contracts are confirmed, but the wire response schema is not. The nested series, ranking, target, freshness, and empty-result fields used by this phase are an explicit frontend hypothesis until Swagger publishes the payload. Do not hand-code chart assumptions into components; normalize through a Zod-validated indicator adapter and fixtures. |
