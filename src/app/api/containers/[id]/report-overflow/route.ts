...
import { getContainerFixture, reportOverflowFixture } from "@/lib/containers-fixtures";
...
export async function POST(
  ...
  try {
    ...
    if (session.mode === "backend-development" && process.env.M6_BACKEND_ORIGIN) {
      const backendResponse = await fetchBackend(request, `/containers/${id}/report-overflow`, "container:report", {
        method: "POST",
      });
    ...
    if (container.status !== "ACTIVE") {
      return errorResponse(
        409,
        `Solo se puede reportar desborde en contenedores activos. Estado actual: ${container.status}`,
        path,
      );
    }

    const updated = reportOverflowFixture(id);
    ...
    return errorResponse(500, "No se pudo reportar el desborde del contenedor.", path);
  }
}
