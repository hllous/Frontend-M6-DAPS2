...
import { getContainerFixture, emptyContainerFixture } from "@/lib/containers-fixtures";
...
export async function POST(
  ...
  try {
    ...
    if (!scenario.capabilities.includes("container:manage")) {
      return errorResponse(403, "No tiene permisos para vaciar contenedores.", path);
    }

    if (session.mode === "backend-development" && process.env.M6_BACKEND_ORIGIN) {
      const backendResponse = await fetchBackend(request, `/containers/${id}/empty`, "container:manage", {
        method: "POST",
      });
    ...
    if (container.status !== "OVERFLOWED") {
      return errorResponse(
        409,
        `Solo se puede vaciar un contenedor en estado desbordado. Estado actual: ${container.status}`,
        path,
      );
    }

    const updated = emptyContainerFixture(id);
    ...
    return errorResponse(500, "No se pudo registrar el vaciado del contenedor.", path);
  }
}
