...
import { AlertTriangle, Check, ChevronLeft, ChevronRight, ListChecks, Paperclip } from "lucide-react";
import { cn } from "@/lib/utils";
import { EvidenceQueue, FieldError, ResultEvidenceTabs, StatusPill, UploadArchitectureNote } from "./components";
import { ZoneResultSchema, type EvidenceUpload } from "./model";
    ...
    <div className="grid min-h-[680px] flex-1 lg:grid-cols-[320px_minmax(0,1fr)]">
      <aside className="border-b border-neutral-200 bg-white lg:border-b-0 lg:border-r">
        <div className="border-b border-neutral-200 px-5 py-5">
          <h2 className="text-xl font-bold tracking-[-0.02em] text-navy">Banco de evidencias</h2>
          <p className="mt-1 text-sm leading-6 text-neutral-600">Una zona activa, con resultado y archivos en el mismo banco de trabajo.</p>
        </div>
        <props.form.Subscribe selector={(state) => state.values.zoneResults}>
          {(zones) => (
            <nav className="p-2" aria-label="Zonas del recorrido">
              {zones.map((zone, index) => (
                ...
                >
                  <span className={cn("flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold", selectedIndex === index ? "bg-white/15" : "bg-neutral-100 text-neutral-600")}>{index + 1}</span>
                  ...
                  {zone.status === "SERVICED" ? <Check className="h-4 w-4" /> : <AlertTriangle className="h-4 w-4 text-amber-400" />}
                </button>
              ))}
            </nav>
          )}
          ...
          {(zone) => (
            <div className="mx-auto max-w-3xl outline-none" data-zone-index={selectedIndex} tabIndex={-1}>
              ...
              <div className="rounded-2xl border border-neutral-200 bg-white p-5 sm:p-7">
                <ResultEvidenceTabs
                  result={<ZoneResultFields form={props.form} index={selectedIndex} evidence={<></>} onZoneServiced={props.onZoneServiced} />}
                  evidence={<EvidenceForZone {...props} zoneId={zone.zoneId} />}
                  showEvidence={props.invalidZoneIndex === selectedIndex && props.invalidField === "evidenceIds"}
                />
              </div>
            </div>
          )}
