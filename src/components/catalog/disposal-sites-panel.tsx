...
export function DisposalSitesPanel({ scenario }: { scenario: OperationalScenario }) {
  ...
  return (
    <section aria-labelledby="disposal-sites-title" className="flex max-w-6xl flex-col gap-6">
      ...
      {state.status === "loading" ? <div aria-label="Cargando sitios de disposición" className="flex flex-col gap-3"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div> : null}
