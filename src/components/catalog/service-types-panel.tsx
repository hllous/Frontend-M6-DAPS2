...
export function ServiceTypesPanel({ scenario }: { scenario: OperationalScenario }) {
  ...
  return (
    <section aria-labelledby="service-types-title" className="flex max-w-6xl flex-col gap-6">
      ...
      {state.status === "loading" ? <div aria-label="Cargando tipos de servicio" className="flex flex-col gap-3"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div> : null}
