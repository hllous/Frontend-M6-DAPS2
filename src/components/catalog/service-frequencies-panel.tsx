  ...
  return (
    <section aria-labelledby="service-frequencies-title" className="flex max-w-6xl flex-col gap-6">
      ...
      <div className="grid gap-3 rounded-xl border border-border bg-card p-4 md:grid-cols-[minmax(180px,1fr)_minmax(180px,1fr)_150px_150px_150px]">
        <Field><FieldLabel htmlFor="filter-frequency-service-type">Tipo de servicio</FieldLabel><select id="filter-frequency-service-type" aria-label="Filtrar por tipo de servicio" className={controlClass} value={serviceTypeId} onChange={(event) => setServiceTypeId(event.target.value)}><option value="">Todos</option>{serviceTypes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
        <Field><FieldLabel htmlFor="filter-frequency-route">Recorrido</FieldLabel><select id="filter-frequency-route" aria-label="Filtrar por recorrido" className={controlClass} value={routeId} onChange={(event) => setRouteId(event.target.value)}><option value="">Todos</option>{routes.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>
      ...
      {state.status === "loading" ? <div aria-label="Cargando frecuencias" className="flex flex-col gap-3"><Skeleton className="h-12 w-full" /><Skeleton className="h-12 w-full" /></div> : null}
