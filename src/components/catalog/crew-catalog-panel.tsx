...
export function CrewCatalogPanel({ scenario }: { scenario: OperationalScenario }) {
  ...
  async function save(event: FormEvent) {
    ...
    try { if (editing) await crewsAdapter.update(editing.id, { name: form.name.trim(), leaderUserId: form.leaderUserId, organizationId: form.organizationId, defaultShift: form.defaultShift, active: editing.active }); else await crewsAdapter.create({ ...form, name: form.name.trim() }); setFormOpen(false); setNotice(editing ? "Cuadrilla actualizada." : "Cuadrilla registrada."); setRequestVersion((version) => version + 1); }
    catch (caught) { setFormError(caught instanceof Error ? caught.message : "No se pudo guardar la cuadrilla."); }
  }
  async function showDetail(crew: Crew) { try { setSelected(await crewsAdapter.get(crew.id)); setMemberSelection([]); setMemberError(null); } catch (caught) { setNotice(caught instanceof Error ? caught.message : "No se pudo cargar el detalle."); } }
  async function addMembers(event: FormEvent) {
    event.preventDefault();
    if (!selected || memberSelection.length === 0) { setMemberError("Seleccione al menos un integrante."); return; }
    setMemberError(null);
    try { setSelected(await crewsAdapter.addMembers(selected.id, memberSelection)); setMemberSelection([]); setNotice("Integrantes agregados."); }
    catch (caught) { setMemberError(caught instanceof Error ? caught.message : "No se pudieron agregar los integrantes."); }
  }
  async function removeMember(userId: string) {
    if (!selected) return;
    try { setSelected(await crewsAdapter.removeMember(selected.id, userId)); setNotice("Integrante quitado."); }
    catch (caught) { setMemberError(caught instanceof Error ? caught.message : "No se pudo quitar el integrante."); }
  }
  ...
  return (
    <section aria-labelledby="crews-title" className={styles.resourcePanel}>
      ...
      <Dialog open={Boolean(selected)} onOpenChange={(open) => { if (!open) setSelected(null); }}><DialogContent><DialogHeader><DialogTitle>Detalle de cuadrilla</DialogTitle><DialogDescription>Las membresías se muestran como referencias de identidad de M1. Office puede administrarlas desde este sub-recurso.</DialogDescription></DialogHeader>{selected ? <div className={styles.detail}><p><strong>{selected.name}</strong></p><p>Responsable: {userName(selected.leaderUserId)}</p><p>Organización: {organizationName(selected.organizationId)}</p><p>Turno: {shiftLabels[selected.defaultShift]}</p><div><strong>Integrantes</strong><ul aria-label="Integrantes de la cuadrilla">{selected.memberUserIds.map((memberUserId) => <li key={memberUserId}>{userName(memberUserId)}{canManage ? <Button type="button" variant="ghost" size="sm" aria-label={`Quitar a ${userName(memberUserId)}`} onClick={() => void removeMember(memberUserId)}><UserMinus data-icon="inline-start" aria-hidden />Quitar</Button> : null}</li>)}</ul></div>{canManage ? <form onSubmit={(event) => void addMembers(event)}><Field><FieldLabel htmlFor="crew-members">Agregar integrantes (M1)</FieldLabel><select id="crew-members" multiple value={memberSelection} onChange={(event) => setMemberSelection(Array.from(event.target.selectedOptions, (option) => option.value))} className={styles.control}>{users.filter((user) => !selected.memberUserIds.includes(user.id)).map((user) => <option key={user.id} value={user.id}>{user.displayName}</option>)}</select><FieldDescription>Seleccione uno o más usuarios administrados por M1.</FieldDescription></Field>{memberError ? <p role="alert" className={styles.formError}>{memberError}</p> : null}<Button type="submit"><UserPlus data-icon="inline-start" aria-hidden />Agregar integrantes</Button></form> : null}</div> : null}<DialogFooter><Button variant="outline" onClick={() => setSelected(null)}>Cerrar</Button></DialogFooter></DialogContent></Dialog>
      <Dialog open={formOpen} onOpenChange={setFormOpen}><DialogContent><DialogHeader><DialogTitle>{editing ? "Editar cuadrilla" : "Registrar cuadrilla"}</DialogTitle><DialogDescription>Los responsables y organizaciones se seleccionan mediante sus IDs de referencia M1.</DialogDescription></DialogHeader>{formError ? <p role="alert" className={styles.formError}>{formError}</p> : null}<form id="crew-form" onSubmit={(event) => void save(event)}><FieldGroup><Field><FieldLabel htmlFor="crew-name">Nombre</FieldLabel><input id="crew-name" value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className={styles.control} required /></Field><Field><FieldLabel htmlFor="crew-type-form">Tipo de cuadrilla</FieldLabel><select id="crew-type-form" value={form.crewType} onChange={(event) => setForm({ ...form, crewType: event.target.value as CrewType })} className={styles.control} disabled={Boolean(editing)} aria-describedby={editing ? "crew-type-locked" : undefined}>{crewTypes.map((type) => <option key={type} value={type}>{crewTypeLabels[type]}</option>)}</select>{editing ? <FieldDescription id="crew-type-locked">El tipo no se puede cambiar después de registrar la cuadrilla.</FieldDescription> : null}</Field><Field><FieldLabel htmlFor="crew-leader">Responsable de cuadrilla (M1)</FieldLabel><select id="crew-leader" value={form.leaderUserId} onChange={(event) => setForm({ ...form, leaderUserId: event.target.value })} className={styles.control} required>{users.map((user) => <option key={user.id} value={user.id}>{user.displayName}</option>)}</select></Field><Field><FieldLabel htmlFor="crew-organization">Organización (M1)</FieldLabel><select id="crew-organization" value={form.organizationId} onChange={(event) => setForm({ ...form, organizationId: event.target.value })} className={styles.control} required>{organizations.map((organization) => <option key={organization.id} value={organization.id}>{organization.displayName}</option>)}</select></Field><Field><FieldLabel htmlFor="crew-shift-form">Turno predeterminado</FieldLabel><select id="crew-shift-form" value={form.defaultShift} onChange={(event) => setForm({ ...form, defaultShift: event.target.value as Shift })} className={styles.control}>{shifts.map((shift) => <option key={shift} value={shift}>{shiftLabels[shift]}</option>)}</select><FieldDescription>La gestión de integrantes queda fuera del alcance de este catálogo.</FieldDescription></Field></FieldGroup></form><DialogFooter><Button type="button" variant="outline" onClick={() => setFormOpen(false)}>Cancelar</Button><Button type="submit" form="crew-form">Guardar cuadrilla</Button></DialogFooter></DialogContent></Dialog>
    </section>
  );
}
