...
- Open the page at both postures, above and below `--bp-field`, and confirm control heights change from 40 px to 48 px, that no viewport scrolls the body horizontally, and that reading text stays at 16 px. A file-level check cannot catch a missing media query; a browser can. This is the one check that must not be skipped — a dropped `@media` block once left this page rendering 40 px controls at Field width, contradicting the rule it exists to teach.
- Verify contrast by measurement, not by eye, whenever a colour changes. Rendered values have matched the computed ones exactly so far, so either method is acceptable as long as one of them is actually run.
- An automated design-lint pass is a useful supplement where the tooling is available locally. It is agent tooling under `.claude/skills/`, which this repository deliberately does not vendor, so treat it as optional and never as the gate. It also degrades to regex matching and silently under-reports unless `htmlparser2`, `css-select`, `css-tree`, and `domutils` are installed alongside it.
